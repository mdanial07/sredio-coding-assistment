...
import * as workHoursData from '../../../../public/assets/work_hours_2020_2024.json';
...
export class HoursService {
  ...
  calculateQuarterlyHoursInRange(startDate: Date | string, endDate: Date | string): any[] {
    const data: any[] = (workHoursData as any).default;
  ...
  getHoursByTypeInDateRange(startDate: string, endDate: string) {
    const dataList: any[] = (workHoursData as any).default;
    ...
    const result = filteredData.reduce((acc: any[], curr: any) => {
      ...
      if (existingType) {
        existingType.hours += curr.hours;
      } else {
        acc.push({ type: curr.type, hours: curr.hours });
      }
      return acc;
    }, []);
  ...
  getCurrentYearHours(startDate: Date, endDate: Date): number {
    const dataList: any[] = (workHoursData as any).default;
  ...
  getProjectedHours(startDate: Date, endDate: Date): number {
    const dataList: any[] = (workHoursData as any).default;
    ...
    const projectedHours = (totalHours / elapsedDays) * totalDaysInRange;
